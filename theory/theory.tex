...
\end{align}
Now let $-N_H \le k, j \le N_H$, and write $\hat k \coloneq k + N_H$ and $\hat j \coloneq j + N_H$.
Then
\begin{align*}
  &\phantom{{}={}} \frac \omega{2 \pi} \paren*{\int_0^{2 \pi / \omega} Q(t)^* Q(t) dt}_{\hat k \hat j} \\
  &= \frac \omega{2 \pi} \int_0^{2 \pi / \omega} \paren*{Q(t)^* Q(t)}_{\hat k \hat j} dt \\
  &= \frac \omega{2 \pi} \int_0^{2 \pi / \omega} \exp\paren{-i k \omega t} \exp\paren{i j \omega t} dt \\
  &= \inpr{\exp\paren{-i k \omega t},\; \exp\paren{i j \omega t}} \\
  &= \delta_{kj}
\end{align*}
...
\begin{align*}
  ...
  \end{bmatrix}
\end{align*}
where $a_{k, j}$ is the $k$th Fourier coefficient of the $j$th degree of freedom.
Then
\begin{align*}
  \Gamma \z =
  \begin{bmatrix}
...
where $\x_j =
\begin{bmatrix}
  x_j(t_0) & x_j(t_1) & \cdots & x_j(t_{N-1})
\end{bmatrix}
$
contains the values of the $j$th degree of freedom of $\x(t)$ at $t = t_0, \ldots, t_{N-1}$.
Write
\begin{align}
  \x_s \coloneq \Gamma \z
\end{align}
