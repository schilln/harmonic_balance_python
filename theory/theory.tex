...
\begin{align*}
  ...
  &= \sum_{k=0}^{N_H} \wh \cb_k \exp\paren{i k \omega t} + \conj{\wh \cb_k \exp\paren{i k \omega t}} \\
  \f\ts{ext}(t)
  &= \db_0 + 2 \sum_{k=1}^{N_H} \Re\bbrack{\db_k \exp\paren{i k \omega t}} \\
  &= \sum_{k=0}^{N_H} \wh \db_k \exp\paren{i k \omega t} + \conj{\wh \db_k \exp\paren{i k \omega t}}.
\end{align*}
...
\begin{align}
  \x(t, \omega)
  &= (\wh Q \otimes I_n) \paren*{\wh \z + \overline {\wh \z}} \\
  \f\ts{nl}(\x, \dot \x, \omega)
  &= (\wh Q \otimes I_n) \paren*{\wh \bb\ts{nl} + \overline {\wh \bb}\ts{nl}} \\
  \f\ts{ext}(t, \omega)
  &= (\wh Q \otimes I_n) \paren*{\wh \bb\ts{ext} + \overline {\wh \bb}\ts{ext}} \\
  \f(t, \x, \dot \x, \omega)
  &= (\wh Q \otimes I_n) \paren*{\wh \bb + \overline {\wh \bb}}.
\end{align}
We also have
\begin{align}
  \label{eq:xp(z)-real}
  \dot \x(t, \omega) &= \omega \paren{\wh Q \nabla \otimes I_n} \paren*{\wh \z + \overline {\wh \z}} \\
  \label{eq:xpp(hat-z)-real}
  \ddot \x(t, \omega) &= \omega^2 \paren*{\wh Q \nabla^2 \otimes I_n} \paren*{\wh \z + \overline {\wh \z}}.
\end{align}
...
\begin{align}
  ...
  \bbrack*{
    ...
    + \wh Q(t) \otimes K
  } \paren*{\wh \z + \overline {\wh \z}}
  = (\wh Q(t) \otimes I_n) \paren*{\wh \bb + \overline {\wh \bb}}.
\end{align}
Defining
\begin{align}
  \widetilde {\wh \nabla}
...
\begin{align}
  ...
  = \wh \bb + \overline {\wh \bb}
\end{align}
and we define $\wh A(\omega) \coloneq \bbrack*{
  \omega^2 \wh \nabla^2 \otimes M
  + \omega \wh \nabla \otimes C
  + I_{N_H + 1} \otimes K
}$.
...
\begin{align}
  ...
  &\coloneq
  \begin{bmatrix}
    I_n \otimes \wh \q(0) & I_n \otimes \wh \q(1) & \cdots & I_n \wh \q(N_H)
  \end{bmatrix}.
\end{align}
Then
\begin{align*}
  \wh \Gamma \paren*{\wh \z + \overline {\wh \z}}
  &= (I_n \otimes \wh \q(0))
  \paren*{\wh \ab_{0} + \overline {\wh \ab}_{0}}
  + \cdots
  + (I_n \otimes \wh \q(N_H))
  \paren*{\wh \ab_{N_H} + \overline {\wh \ab}_{N_H}} \\
  &= \x_s.
\end{align*}
Similarly, defining
\begin{align}
...
\begin{align*}
  ...
  \begin{bmatrix}
    1/2 & 1 & \cdots & 1
  \end{bmatrix}\trp \odot \paren*{\wh \Gamma^\dagger \x_s} \\
  &=
  \begin{bmatrix}
    1/2 & 1 & \cdots & 1
  \end{bmatrix}\trp \odot \z \\
  &= \wh \z.
\end{align*}
We then can write
\begin{align}
  \label{eq:x_s-real}
  \x_s &= \wh \Gamma \paren*{\wh \z + \overline {\wh \z}} \\
  \label{eq:xp_s-real}
  \dot \x_s &= \omega \wh \Gamma (\wh \nabla \otimes I_n) \paren*{\wh \z + \overline {\wh \z}} \\
  \ddot \x_s &= \omega^2 \wh \Gamma (\wh \nabla^2 \otimes I_n) \paren*{\wh \z + \overline {\wh \z}}.
\end{align}
...
\begin{align}
  ...
  &= \diag(1/2, 1, \ldots, 1) \;
  \Gamma^\dagger \; \f\ts{nl} \; \paren*{
    \Gamma \paren*{\wh \z + \overline {\wh \z}},\;
    \omega \Gamma (\nabla \otimes I_n) \paren*{\wh \z + \overline {\wh \z}}
  }.
\end{align}
...
\begin{align}
  \wh \Rb(\wh \z)
  &\coloneq \wh A(\omega) \paren*{\wh \z + \overline {\wh \z}}
  + \paren*{\wh \bb\ts{nl} + \overline {\wh \bb}\ts{nl}}
  - \paren*{\wh \bb\ts{ext} + \overline {\wh \bb}\ts{ext}}.
\end{align}
We need to compute
% TODO: Is this differentiation right? And the same question for the nonlinear force...
\begin{align*}
  \wh \Rb_{\wh \z}(\wh \z) = \wh A(\omega) + \frac {d \wh \bb\ts{nl}(\wh \z)}{d \wh \z}.
\end{align*}
Observe,
\begin{align*}
  \frac {d \wh \bb\ts{nl}(\wh \z)}{d \wh \z}
  &= \frac d{d \wh \z} \bigg[
    \diag(1/2, 1, \ldots, 1) \Gamma^\dagger \f\ts{nl} \paren{\x_s,\; \dot \x_s}
  \bigg] \\
  &= \diag(1/2, 1, \ldots, 1) \Gamma^\dagger \frac d{d \wh \z} \bigg[
    \f\ts{nl} \paren{\x_s,\; \dot \x_s}
  \bigg] \\
  &= \diag(1/2, 1, \ldots, 1) \Gamma^\dagger \bigg[
    \frac d{d \x} \f\ts{nl} \paren{\x_s,\; \dot \x_s} \frac {d \x_s}{d \wh \z}
    + \frac d{d \dot \x} \f\ts{nl} \paren{\x_s,\; \dot \x_s} \frac {d \dot \x_s}{d \wh \z}
  \bigg] \\
  &= \diag(1/2, 1, \ldots, 1) \Gamma^\dagger \bigg[
