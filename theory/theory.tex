...
Note
\begin{align*}
  P_\z(\y_{i+1}^k, s) &= 2 \paren*{\z_{i+1}^k - \z_i}\trp \\
...
Define
\begin{equation}
  P(\z_{i+1}^k, \omega_{i+1}^k) \coloneq \vb_i\trp \paren*{\y_{i+1}^k - \y_{i+1}^0}
\end{equation}
...
However, I haven't gotten arclength continuation to continue past a fold bifurcation in the NLFR curve, and my implementation of pseudo-arclength continuation is yielding mixed and irregular results.
Though I haven't implemented either method without the real assumption, so I don't have a successful baseline to compare against in either case.
