...
\begin{align*}
  \x(t)
  &\approx \sum_{k=0}^{N_H} \widehat \ab_k \exp\paren{i k \omega t} + \conj{\widehat \ab_k \exp\paren{i k \omega t}}.
\end{align*}
We can write the forces similarly:
\begin{align*}
  \f\ts{nl}(\x, \dot \x)
  &= \cb_0 + 2 \sum_{k=1}^{N_H} \Re\bbrack{\cb_k \exp\paren{i k \omega t}} \\
  &= \sum_{k=0}^{N_H} \widehat \cb_k \exp\paren{i k \omega t} + \conj{\widehat \cb_k \exp\paren{i k \omega t}} \\
  \f\ts{ext}(t)
  &= \db_0 + 2 \sum_{k=1}^{N_H} \Re\bbrack{\db_k \exp\paren{i k \omega t}} \\
  &= \sum_{k=0}^{N_H} \widehat \db_k \exp\paren{i k \omega t} + \conj{\widehat \db_k \exp\paren{i k \omega t}}.
\end{align*}
We store the Fourier coefficients
\begin{align*}
  \widehat \z &\coloneq
  \begin{bmatrix}
  ...
  &=
  \begin{bmatrix}
    \widehat \ab_0\trp & \widehat \ab_1\trp & \cdots & \widehat \ab_{N_H}\trp
  \end{bmatrix}\trp \\
  ...
  &=
  \begin{bmatrix}
    \widehat \cb_0\trp & \widehat \cb_1\trp & \cdots & \widehat \cb_{N_H}\trp
  \end{bmatrix}\trp \\
  ...
  &=
  \begin{bmatrix}
    \widehat \db_0\trp & \widehat \db_1\trp & \cdots & \widehat \db_{N_H}\trp
  \end{bmatrix}\trp
\end{align*}
and again write $\widehat \bb(\z, \omega) = \widehat \bb\ts{ext}(\omega) - \widehat \bb\ts{nl}(\z)$.

\subsection{Projecting the governing equation}

Then writing
\begin{align}
  \widehat Q(t, \omega) \coloneq
  \begin{bmatrix}
...
\begin{align}
  \x(t, \omega)
  &= (\widehat Q \otimes I_n) \paren*{\widehat \z + \overline {\widehat \z}} \\
  \f\ts{nl}(\x, \dot \x, \omega)
  &= (\widehat Q \otimes I_n) \paren*{\widehat \bb\ts{nl} + \overline {\widehat \bb}\ts{nl}} \\
  \f\ts{ext}(t, \omega)
  &= (\widehat Q \otimes I_n) \paren*{\widehat \bb\ts{ext} + \overline {\widehat \bb}\ts{ext}} \\
  \f(t, \x, \dot \x, \omega)
  &= (\widehat Q \otimes I_n) \paren*{\widehat \bb + \overline {\widehat \bb}}.
\end{align}
We also have
\begin{align}
  \label{eq:xp(z)-real}
  \dot \x(t, \omega) &= \omega \paren{\widehat Q \nabla \otimes I_n} \paren*{\widehat \z + \overline {\widehat \z}} \\
  \label{eq:xpp(hat-z)-real}
  \ddot \x(t, \omega) &= \omega^2 \paren*{\widehat Q \nabla^2 \otimes I_n} \paren*{\widehat \z + \overline {\widehat \z}}.
\end{align}
...
\begin{align}
  ...
  \bbrack*{
    ...
    + \omega \paren*{Q(t) \nabla} \otimes C
    + Q(t) \otimes K
  } \paren*{\widehat \z + \overline {\widehat \z}}
  = (Q(t) \otimes I_n) \paren*{\widehat \bb + \overline {\widehat \bb}}.
\end{align}
Defining 
\begin{align}
  \widetilde {\widehat \nabla}
  ...
  \end{bmatrix} \\
  \label{eq:nabla-real}
  \widehat \nabla &\coloneq \diag{\widetilde {\widehat \nabla}},
\end{align}
Fourier--Galerkin projection then transforms this equation to
\begin{align}
  \bbrack*{
    \omega^2 \widehat \nabla^2 \otimes M
    + \omega \widehat \nabla \otimes C
  ...
  = \widehat \bb + \overline {\widehat \bb}
\end{align}
and we define $\widehat A(\omega) \coloneq \bbrack*{
  \omega^2 \widehat \nabla^2 \otimes M
  + \omega \widehat \nabla \otimes C
  + I_{N_H + 1} \otimes K
}$.
...
\begin{align}
  ...
  &\coloneq
  \begin{bmatrix}
    I_n \otimes \widehat \q(0) & I_n \otimes \widehat \q(1) & \cdots & I_n \widehat \q(N_H)
  \end{bmatrix}.
\end{align}
Then
\begin{align*}
  \widehat \Gamma \paren*{\widehat \z + \overline {\widehat \z}}
  &= (I_n \otimes \widehat \q(0))
  \paren*{\widehat \ab_{0} + \overline {\widehat \ab}_{0}}
  + \cdots
  + (I_n \otimes \widehat \q(N_H))
  \paren*{\widehat \ab_{N_H} + \overline {\widehat \ab}_{N_H}} \\
  &= \x_s.
\end{align*}
Similarly, defining
\begin{align}
  \widehat \Gamma^\dagger &\coloneq \frac1N
  \begin{bmatrix}
    I_n \otimes \q(0)^* \\
    \vdots \\
    I_n \otimes \q(N_H)^*
...
\begin{align*}
  \widehat \Gamma \x_s &= \z.
\end{align*}
Note this is $\z$, \textit{not} $\widehat \z$.
We then can write
\begin{align}
  \label{eq:x_s-real}
  \x_s &= \widehat \Gamma \paren*{\widehat \z + \overline {\widehat \z}} \\
  \label{eq:xp_s-real}
  \dot \x_s &= \omega \widehat \Gamma (\widehat \nabla \otimes I_n) \paren*{\widehat \z + \overline {\widehat \z}} \\
  \ddot \x_s &= \omega^2 \widehat \Gamma (\widehat \nabla^2 \otimes I_n) \paren*{\widehat \z + \overline {\widehat \z}}.
\end{align}
